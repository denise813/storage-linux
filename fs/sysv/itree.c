 ...
 * 创建日期  : 2019年10月13日
 * 函数功能  : 文件块编号推导磁盘块编号
        文件开始的文件块编号到相对于块设备开始的逻辑块编号的映射
 * 输入参数  : struct inode *inode            指向文件inode的指针
               sector_t iblock                文件块编号
               struct buffer_head *bh_result  指向记录块映射的buffer_head的指针
               int create                     在没有映射的时候创建映射
 * 输出参数  : 无
 * 返 回 值  : static
 * 调用关系  : 
 * 其    它  : 
