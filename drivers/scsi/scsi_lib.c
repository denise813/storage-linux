 ...
 */
/*****************************************************************************
 * 函 数 名  : __scsi_execute
 * 负 责 人  : hy
 * 创建日期  : 2019年10月16日
 * 函数功能  : 发送scsi命令
 * 输入参数  : struct scsi_device *sdev      指向SCSI设备描述符的
               const unsigned char *cmd      要执行的SCSI命令字符串
               int data_direction            数据传输方向
                      DMA_FROM_DEVICE 设备读取
                      DMA_TO_DEVICE分 设备写入
                      DMA_NONE 没有数据传输
               void *buffer                  指向保存结果的缓冲区指针
               unsigned bufflen              缓冲区长度
               unsigned char *sense          指向用于保存标准格式的感测信息的缓冲区
               struct scsi_sense_hdr *sshdr  指向用于保存标准格式的感测头信息的缓冲区
               int timeout                   秒为单位的请求超时时间
               int retries                   重试请求的次数
               u64 flags                     剩余长度
...
/** comment by hy 2018-10-16
 * # blk_rq_map_kern 内核数据映射到request
    映射内核数据到一个块设备驱动层请求
    blk_rq_map_kern() 用于REQ_TYPE_BLOCK_PC
    映射用户数据到一个块设备驱动层请求，
    blk_rq_map_user() 用于REQ_TYPE_BLOCK_PC
    映射一个块设备驱动层请求到聚散列表
    blk_rq_map_sg()
    映射块设备驱动层请求中的完整性向量到聚散列表
 ...
 */
	 ...
	 */
	blk_execute_rq(req->q, NULL, req, 1);/*1 表示需要优先执行
	        NULL 不能认定这是一个SCSI磁盘类设备 或者SCSI磁盘驱动已经被加载*/
...
/** comment by hy 2018-10-16
 * # 查看SCSI命令的result域及感测数据以便确定如何处理这条命令
 */
	...
	switch (disposition) {
		...
		case NEEDS_RETRY:
/** comment by hy 2018-10-16
 * # 将SCSI命令通过对应块设备驱动器请求的连接件重新链入SCSI设备的请求队列
 */
