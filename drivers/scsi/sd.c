 ...
 */
/*****************************************************************************
 * 函 数 名  : sd_spinup_disk
 * 负 责 人  : hy
 * 创建日期  : 2019年10月16日
 * 函数功能  : 使磁盘转起来
 * 输入参数  : struct scsi_disk *sdkp   
 * 输出参数  : 无
 * 返 回 值  : 
 * 调用关系  : 
 * 其    它  : 
 ...
 * # 目的为让SCSI磁盘“准备好”给予足够的等待时间
     在发送START_STOP命令之后，SCSI磁盘“转动起来”需要一段时间
     某些USB闪存设备， 它们的固件运行速度很慢
     上述很慢的情况设置为1
 */
	spintime = 0;
 ...
 */
		 ...
		 */
/** comment by hy 2018-10-16
 * # 等待磁盘转动起来,但SCSI设备不希望发送自动启动命令
 */
		if (sdkp->device->no_start_on_add)
			break;

/** comment by hy 2018-10-16
 * # 感测数据为NOT_READY， 向SCSI磁盘发送START UNIT命令
     此之前需要排除三种情况
 */
		if (sense_valid && sshdr.sense_key == NOT_READY) {
			if (sshdr.asc == 4 && sshdr.ascq == 3) /*需要手工介入*/
				break;	/* manual intervention required */
			if (sshdr.asc == 4 && sshdr.ascq == 0xb)  /*磁盘正待机*/
				break;	/* standby */
			if (sshdr.asc == 4 && sshdr.ascq == 0xc)  /*磁盘不可用*/
				break;	/* unavailable */
 ...
 **/
/*****************************************************************************
 * 函 数 名  : sd_revalidate_disk
 * 负 责 人  : hy
 * 创建日期  : 2019年10月15日
 * 函数功能  :  
 * 输入参数  : struct gendisk *disk  重新校验磁盘
 * 输出参数  : 无
 * 返 回 值  : static
 * 调用关系  : 
 * 其    它  : 
...
/** comment by hy 2018-10-16
 * # 尝试等到SCSI磁盘已经准备好， 必要时使磁盘转起来
     前者发送TEST UNIT READY命令， 后者发送START UNIT命令
 */
	sd_spinup_disk(sdkp);
	...
	if (sdkp->media_present) {
/** comment by hy 2018-10-16
 * # 读取SCSI磁盘的容量 查询VPD 0xB0确定设备期望的I/O长度
 */
...
/** comment by hy 2018-10-16
 * # 读取SCSI磁盘的ATO位,确定操作系统释放可以访问它的DIF三元组中的应用程序标签
     通过向SCSI磁盘发送命令， 并查询返回的数据格式
 */
...
/** comment by hy 2018-10-16
 * # 清除SCSI磁盘描述符的初始值为1的first_scan域
    判断sd_revalidate_disk函数及它调用到的上述函数
    到底是在第一次运行时 add_disk之前 ， 还是在第二次运行时
 */
	sdkp->first_scan = 0;
/** comment by hy 2018-10-16
 * # 根据通过INQUIRY命令查询到的SCSI设备的容量
     设置调用 set_capacity 通用磁盘结构中的capacity
 */
 ...
 * 创建日期  : 2019年10月15日
 * 函数功能  :  
 * 输入参数  : struct device *dev  磁盘驱动入口在SCSI中间层调用
        在给定的<host,channel,id,lun> 和新的设备之间建立映射
        更准确的说选定块设备编号和次设备编号
 * 输出参数  : 无
 * 返 回 值  : static
 * 调用关系  : 
...
/** comment by hy 2018-10-15
 * # SCSI磁盘分配设备名，设备名命令为sda～sdz
     设备预留了16个设备编号， 
     其中第一个编号用来表示这个完整的SCSI磁盘，
     而后面15个编号用来表示磁盘上的可能分区
     注设备号|次设备号|分区号
     major1  |
     预留设备主设备号为 major0 为16个
     设备在主设备号0和1 之间交替排放
     bit4到bit7（(index & 0xf0)>>4） 取对应的预留主设备号，
     而用其他位来计算次设备号
 */
...
/** comment by hy 2018-10-09
 * # 构建SCSI磁盘各种结构关系中几个比较重要的部分指针赋值
     在sd_probe_async函数通过driver_data指向
     private_data等各种域，
 */
	sdkp->device = sdp;
...
/** comment by hy 2018-10-15
 * # SCSI磁盘在sysfs文件系统中创建目录
     内嵌设 class 为sd_disk_class 且parent没有 class，
     该目录会在SCSI设备目录中创建在以类名 scsk_disk 0 为名字的子目录下，
     目录名字被设置成SCSI设备名。 举例说明，
     /sys/devices/pci0000:00/0000:00:10.0/host2/target2:0:0/2:0:0:0/scsi_disk/2:0:0:0
 */
	...
	error = device_add(&sdkp->dev); /* 添加到系统 */
	if (error)
		goto out_free_index;
...
/** comment by hy 2018-10-09
 * # 当匹配到了一个scsi_device，
   # 而且此scsi设备的类型域是scsi disk时，
   # 就触发sd_probe分配scsi_disk结构和gendisk结构，
   # 然后建立起对上（gendisk)和对下（scsi_device)的关系
 */
