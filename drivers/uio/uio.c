 ...
 * 负 责 人  : hy
 * 创建日期  : 2018年10月18日
 * 函数功能  : 获得和次设备号关联的uio_device指针，
                创建辅助变量listener, 
                调用info指向的uio_info结构中的open方法
 ...
 */
	...
	listener->dev = idev;
/** comment by hy 2018-10-18
 * # 获得uio_device 指向设备的事件计数值
 */
	listener->event_count = atomic_read(&idev->event);
/** comment by hy 2018-10-18
 * # 把listener指针保存到filep->private_data
 */
 ...
 * 负 责 人  : hy
 * 创建日期  : 2018年10月18日
 * 函数功能  : 进程在传递到该系统调用的所有文件描述符对应的等待队列上等待
                  可以立即无阻塞执行的位掩码
 * 输入参数  : struct file *filep   
               poll_table *wait     
 * 输出参数  : 无
 * 返 回 值  : static
 * 调用关系  : 
...
/** comment by hy 2018-10-18
 * # 向poll_table类型的wait表中添加uio_device类型指针指向结构的wait等待队列
 */
	poll_wait(filep, &idev->wait, wait);
/** comment by hy 2018-10-18
 * # istener中的事件计数值event_count和uio_device的事件计数值count不一致时
 */
 ...
 */
	...
	struct uio_device *idev = listener->dev;
/** comment by hy 2018-10-18
 * # 利用listener指针找到指向uio_device类型结构指针创建一个等待队列的项
 */
 ...
 * 负 责 人  : hy
 * 创建日期  : 2018年10月18日
 * 函数功能  : 读取用户空间的值，并调用uio_device注册的irqcontrol函数
 * 输入参数  : struct file *filep       
...
/*****************************************************************************
 * 函 数 名  : init_uio_class
 * 负 责 人  : hy
 * 创建日期  : 2018年10月18日
 * 函数功能  : 申请字符设备号，设备，并注册到系统中
        创建 /sys/class/uio
 * 输入参数  :    
 * 输出参数  : 无
 * 返 回 值  : static
