...
struct config_item {
	char			*ci_name; /*目录名称,指向 ci_namebuf*/
	char			ci_namebuf[CONFIGFS_ITEM_NAME_LEN]; /*存储名字的空间*/
	struct kref		ci_kref;
	struct list_head	ci_entry;
	struct config_item	*ci_parent;
	struct config_group	*ci_group;
	const struct config_item_type	*ci_type; /*目录下属性和属性操作，属性表现为目录下的文本文件*/
...
struct config_item_type {
	...
	struct configfs_item_operations		*ct_item_ops; /*当前属性操作方法*/
	struct configfs_group_operations	*ct_group_ops;  /*当前目录操作方法*/
	struct configfs_attribute		**ct_attrs; /*当前的目录属性数组*/
 ...
 */
struct config_group {
	struct config_item		cg_item;  /*表示一个目录*/
...
struct configfs_attribute {
	...
	ssize_t (*show)(struct config_item *, char *);  /*读*/
	ssize_t (*store)(struct config_item *, const char *, size_t); /*写*/
};
...
struct configfs_subsystem {
/** comment by hy 2018-10-21
 * # 抽象容器 可以包括config_item，还可以递归包含config_group
 */
