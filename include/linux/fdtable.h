...
struct fdtable {
	...
	struct file __rcu **fd;      /* current fd array */ /*文件描述符数组*/
 ...
 * # 打开文件描述符位图
 */
	unsigned long *open_fds;  /*文件描述符指针数组open_fds位图*/
	unsigned long *full_fds_bits;
	struct rcu_head rcu;  /*用于RCU机制的域*/
};
...
struct files_struct {
   ...
   */
	...
	struct fdtable __rcu *fdt;  /*指向打开文件表的指针*/
	struct fdtable fdtab; /*内嵌的打开文件表*/
  /*
   * written part on a separate cache line in SMP
   */
	spinlock_t file_lock ____cacheline_aligned_in_smp;
	unsigned int next_fd; /*已分配的文件描述符加 1*/
